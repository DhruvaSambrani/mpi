...
int main(int argc, char *argv[])
{
  ...
  FILE *phi_corr_av_err = fopen("phi_corr_av_err-0.txt", "a"); // write only

  if (phi_corr_av_err == NULL)
  {
  ...
  for (i = 0; i < ncols; i++)
  {
    ...
    else
    {
      ...
      if (block)
      {
        ...
        fprintf(phi_corr_av_err, " %0.10g %.10g  %.8g\n", alpha, ave, sig);
        //printf(" %d  %.10g  %.8g\n",col[i],ave,sig);
      }
      else
      {
        ...
        if (i == 0)
          printf(" col - average - sigma - tau_int\n");
        fprintf(phi_corr_av_err, " %0.10g %.10g  %.8g\n", alpha, ave, sig);
        //printf(" %d  %.10g  %.8g  %g\n",col[i],ave,sig,tint);
      }
    }
